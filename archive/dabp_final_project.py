...
print(hospitals, schools, simulation)
...
y = m.addVars(schools, hospitals, GRB.INTEGER, lb = 0.0)
...
#two constraints required
for j in schools:
   for i in hospitals:
       m.addConstr(y[j, i] <= x[j, i]/12)
       m.addConstr(y[j, i] + 2 >= x[j, i]/12 + .0000001)
